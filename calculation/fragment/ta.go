...
func calculateFirstTA(headMass float64, polyamine, following models.Polyamine) float64 {
	return headMass + polyamine.Mass + following.Sub.Mass - float64(following.Quaternary)*calculation.H + calculation.NH3 - massElectron
}
...
func calculateMiddleTAs(previous float64, polyamine, following models.Polyamine) float64 {
	return previous + polyamine.Mass - polyamine.Sub.Mass + following.Sub.Mass - float64(following.Quaternary)*calculation.H - massElectron
}
...
func calculateLastTA(previous float64, polyamine models.Polyamine, tail models.Tail) float64 {
	return previous + polyamine.Mass - polyamine.Sub.Mass + tail.Sub.Mass - float64(tail.Quaternary)*calculation.H - massElectron
}
